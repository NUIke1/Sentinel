    ...
    oldnamecall = hookmetamethod(game, "__namecall", newcclosure(function(...)
        ...
        if self == workspace and not checkcaller() and method == "Raycast" then
            ...
            if hitpart then
                ...
                return oldnamecall(self, origin, direction)
            end
        end
...
local function create_tracer(type, parent, settings)
    ...
    local function update_lines()
        ...
        for _, player in pairs(players:GetPlayers()) do
            if player ~= LocalPlayer then
                ...
                if char then
                    ...
                    if target_part then
                        local screen_point, on_screen = Cam:WorldToScreenPoint(target_part.Position)
                        if on_screen then
        ...
        for i = 1, #targets do
            ...
            if settings.Tracer.Properties.Mode == "From Client" and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local humanoid_root_part = LocalPlayer.Character.HumanoidRootPart
                local screen_point, on_screen = Cam:WorldToScreenPoint(humanoid_root_part.Position)

                if on_screen then
                    origin_position = Vector2.new(screen_point.X, screen_point.Y)
                end
            elseif settings.Tracer.Properties.Mode == "From Screen" then
                origin_position = Vector2.new(Cam.ViewportSize.X / 2, Cam.ViewportSize.Y)
...
local DrawingImagesEnum = {
    "rbxassetid://9051927749",
    "rbxassetid://9051988128",
    "rbxassetid://9051998433",
    "rbxassetid://9051961469",
    "rbxassetid://9051968607",
    "rbxassetid://9052002146",
    "rbxassetid://9052009535",
    "rbxassetid://9051990418",
    "rbxassetid://9052018972",
    "rbxassetid://9052006445",
    "rbxassetid://9051994414",
    "rbxassetid://9051977914",
    "rbxassetid://9051982241",
    "rbxassetid://9051957557",
    "rbxassetid://9051974864"
}
...
local function create_esp(player)
    ...
    create_highlight(Default, player.Character or player.CharacterAdded:Wait())
end
...
EspTab:Toggle({
    ...
    Value = true,
    Callback = function(state)
        Default.Box.Properties.On = state
        Default.Box.Outline.Visible = state
        Default.Box.Inline.On = state
        Default.Box.InlineOutline.On = state
        Default.Text.Display.Properties.On = state
        Default.Text.Tool.Properties.On = state
        Default.Text.Studs.Properties.On = state
        Default.Text.Health.Properties.On = state
        Default.Text.Armor.Properties.On = state
        Default.HealthBar.Properties.On = state
        Default.ArmorBar.Properties.On = state
        Default.Skeleton.Properties.On = state
        Default.Tracer.Properties.On = state
        Default.Chams.Properties.On = state
        Default.Aura.Visible = state
        Default.FOV.Properties.On = state
        Default.Crosshair.On = state
        Default.Trail.Properties.On = state
        Default.Icon.WeaponIcon.Properties.On = state
        espStates.Box = state
        espStates.Name = state
        espStates.Health = state
        espStates.Distance = state
        espStates.Weapon = state
        espStates.HealthBar = state
        espStates.ArmorBar = state
        espStates.Skeleton = state
        espStates.Tracer = state
        espStates.Chams = state
        espStates.Aura = state
        espStates.FOV = state
        espStates.Crosshair = state
        espStates.Trail = state
        espStates.Watermark = state
        WindUI:Notify({
            Title = "ESP",
...
EspTab:Toggle({
    ...
    Value = true,
    Callback = function(state)
        espStates.Inline = state
        Default.Box.Inline.On = state
        Default.Box.Inline.Visible = state
    end
})
...
EspTab:Toggle({
    ...
    Value = true,
    Callback = function(state)
        espStates.InlineOutline = state
        Default.Box.InlineOutline.On = state
        Default.Box.InlineOutline.Visible = state
    end
})
...
EspTab:Colorpicker({
    ...
    Callback = function(color)
        Default.Box.Properties.Color = color
    end
})
...
EspTab:Colorpicker({
    ...
    Default = Color3.fromRGB(255, 255, 255),
    Callback = function(color)
        Default.Text.Display.Properties.Color = color
    end
})