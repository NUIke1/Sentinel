        ...
        if espActive[name] then
            for _, entity in ipairs(workspace:GetDescendants()) do
                if entity.Name == name and entity:IsA("Model") then
                    CreateESP(entity, monsterNames[name] or name, colors.monster, colors.monsterText)
                end
            end
        end
    end
        ...
        if espActive[objName] then
            for _, obj in ipairs(workspace:GetDescendants()) do
                ...
                if objName == "Door" and obj.Name == "Door" and obj:FindFirstChild("Door") then
                    CreateESP(obj.Door, "门", colors.world, colors.otherText)
                end
                if objName == "Padlock" and obj.Name == "Padlock" then
                    CreateESP(obj, "挂锁", colors.world, colors.otherText)
                end
            end
        end
    end
        ...
        if espActive[name] then
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj:IsA("Model") and obj.Name == name then
                    CreateESP(obj, itemNames[name] or name, colors.item, colors.otherText)
                end
            end
        end
    end
    ...
    if espActive.GoldPile then
        for _, obj in ipairs(workspace:GetDescendants()) do
            if obj:IsA("Model") and obj.Name == "GoldPile" then
                local val = obj:GetAttribute("GoldValue") or ""
                CreateESP(obj, "💰金币 " .. tostring(val), colors.gold, colors.otherText)
            end
        end
    end
end
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character then
                ...
                if humanoid and humanoid.Health > 0 then
                    CreateESP(player.Character, player.Name, colors.player, colors.otherText)
                else
...
local function UpdateAllESP()
    ...
    if espActive.Player then ScanPlayers() end
    for _, name in ipairs(monsters) do if espActive[name] then ScanMonsters() break end end
    for _, name in ipairs(items) do if espActive[name] then ScanItems() break end end
    if espActive.GoldPile then ScanGold() end
    for objName, _ in pairs(worldItems) do if espActive[objName] then ScanWorldItems() break end end
